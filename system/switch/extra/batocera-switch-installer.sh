...
######################################################################
#--------------------------------------------------------------------- 
APPNAME="SWITCH-EMULATION PARA 41" 
...
if [[ -f "$cfg" ]]; then 
      # verificar versão do arquivo de configuração & atualizar ---------------------------
      ...
      wget -q --no-check-certificate --no-cache --no-cookies -O "/tmp/.CONFIG.txt" "$link_defaultconfig"
         ...
         latestver=$(cat "/tmp/.CONFIG.txt" | grep "(ver " | head -n1 | sed 's,^.*(ver ,,g' | cut -d ")" -f1)
            if [[ "$latestver" > "$currentver" ]]; then 
               ...
               echo -e "\n~/switch/CONFIG.txt ARQUIVO ATUALIZADO!\n"
            fi
      # verificar versão do arquivo de configuração & atualizar ---------------------------
   cp $cfg /tmp/.userconfigfile 2>/dev/null
fi
# -------------------------------------------------------------------- 
# PURGAR INSTALAÇÕES ANTIGAS 
...
# -------------------------------------------------------------------- 
# PREENCHER /USERDATA/SYSTEM/SWITCH/CONFIGGEN/GENERATORS/CITRON
...
# -------------------------------------------------------------------- 
# PREENCHER /USERDATA/SYSTEM/SWITCH/CONFIGGEN/GENERATORS/EDEN
...
# -------------------------------------------------------------------- 
# PREENCHER /USERDATA/SYSTEM/SWITCH/CONFIGGEN/GENERATORS/SUDACHI
...
# -------------------------------------------------------------------- 
# # PREENCHER /USERDATA/SYSTEM/.LOCAL/SHARE/APPLICATIONS
...
sleep 0.1
   # --- \ restaurar arquivo de configuração do usuário para o atualizador se executando instalação limpa/atualização do instalador switch 
...
if [[ -e /userdata/system/switch/extra/installation ]]; then
    ...
    echo 
    echo 
    echo -e "   ${PURPLE}INFORMAÇÃO! ${PURPLE}"
    echo -e "   ${PURPLE}USERDATA DEVE ESTAR EM EXT4/BTRFS! PARA FUNCIONAR A EMULAÇÃO SWITCH ${PURPLE}"
    echo -e "   ${PURPLE}NÃO HÁ AJUDA SE VOCÊ NÃO ESTIVER EM EXT4/BTRFS! ${PURPLE}"
    echo -e "   ${PURPLE}SE VOCÊ JÁ ESTÁ EM BTRFS/EXT4 PODE IGNORAR ESTA MENSAGEM ${PURPLE}"
    echo 
    ...
    echo -e "   ${X}> Após isso execute SWITCH UPDATER em PORTAS ${X}" 
    echo
    echo
    echo -e "   ${X}-------------------------------------------------------------------${X}"
    ...
    echo -e "   ${X}-------------------------------------------------------------------${X}"
    echo 
    echo -e "   ${GREEN}RECARREGUE SUA LISTA DE JOGOS E APROVEITE${GREEN}" 
    echo 
    echo -e "   ${GREEN}Esta página será fechada automaticamente em 10 segundos...${GREEN}"
    ...
    echo
    echo
    echo -e "   ${X}Se ainda falhar, tente instalar usando este comando: "
    echo
